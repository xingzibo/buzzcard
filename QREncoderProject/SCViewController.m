...
- (void)addMyButton{    // Method for creating button, with background image and other properties
    ...
    [button setTitle:@"Cancel Scan" forState:UIControlStateNormal];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    for (AVMetadataObject *metadata in metadataObjects) {
        if ([metadata.type isEqualToString:AVMetadataObjectTypeQRCode]) {
           ...
           // NSLog(@"the decoded msg is %@", _decodedMessage.text);
         ...
         // ContactDetailViewController *tvc = [self.storyboard instantiateViewControllerWithIdentifier:@"ContactDetails"];
         // [self presentViewController:tvc animated:YES completion:nil];
